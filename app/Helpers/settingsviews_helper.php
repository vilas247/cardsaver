 ...
 * Class SettingsViews
 *
 * Represents a helper class to connect CARDSAVER & BigCommerce 
...
class SettingsViews
{
	...
	{		
		...
		if(!empty($email) && !empty($accessToken) && !empty($storeHash)){
			...
			if (count($result) > 0) {
				...
				$builderinsert = $db->table('cardsaver_token_validation'); 
	...
	public static function deleteScripts($sellerdb,$acess_token,$store_hash,$email_id,$validation_id){
		$db = \Config\Database::connect();
		$builder = $db->table('cardsaver_scripts');        
		$builder->select('*');       
		...
		if (count($result) > 0) {
			foreach($result as $k=>$v){
				...
				$builderdelete = $db->table('cardsaver_scripts'); 
				...
				if (strpos($response->getHeader('content-type'), 'application/json') != false){
					...
					if(!empty($response)){
						...
						if(isset($response['data']['uuid'])){
							...
							$builderinsert = $db->table('cardsaver_scripts'); 
							$builderinsert->insert($data);
							$rStatus++;
						}
					}
