 ...
 */
class Cardsaverpay extends BaseController
{
	/**
	 * Index - default page
	 *
			...
			if (filter_var($email_id, FILTER_VALIDATE_EMAIL)) {
				...
				if (count($result) > 0) {
					...
					$req = array(
						...
						'redirectURL' => getenv('app.baseURL').'cardsaverpay/success'////,
	...
	public function success()
	{
		...
		$db = \Config\Database::connect();
		log_message('info', 'PostLink Update Order Cardsaver Payment');
					...
					if(isset($res['secure_url'])){
						...
						if(isset($invoice_result[0])) {
							...
							$bg_customer_id = $invoice_result['bg_customer_id'];
								...
								$url = $res['secure_url'].'/cardsaver-order-confirmation?authKey='.$invoice_id;
								echo '<script>window.parent.location.href="'.$url.'";</script>';
						}else{
							$url = $res['secure_url']."/checkout?cardsaverinv=".base64_encode(json_encode($invoice_id));
							echo '<script>window.parent.location.href="'.$url.'";</script>';
						}
					}
				}
